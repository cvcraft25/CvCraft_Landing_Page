...
export default function CreateDefaultNotFoundPage() {
  ...
  const missingPath = useMemo(() => location.pathname.replace(/^\//, ''), [location.pathname]);
  // In SPA mode, there's no route loader. Optionally surface discovered pages using Vite glob.
  const existingRoutes = useMemo(() => {
    try {
      const modules = import.meta.glob('/src/**/page.{js,jsx,ts,tsx}', { eager: false });
      const matches = Object.keys(modules).sort((a, b) => a.length - b.length);
      ...
      return [] as { url: string; path: string }[];
    }
  }, []);
