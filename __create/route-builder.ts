...
function getHonoPath(routeFile: string): { name: string; pattern: string }[] {
  ...
  // Normalize Windows path separators to POSIX for consistent splitting
...
async function registerRoutes() {
  ...
  for (const routeFile of routeFiles) {
    try {
      // Convert absolute file path to a file URL so Vite's module runner can resolve it reliably on Windows
